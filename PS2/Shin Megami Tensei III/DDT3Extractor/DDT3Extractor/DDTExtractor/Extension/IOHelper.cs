...
namespace AtlusGames.Extension
{
    ...
    public class IOHelper
    {
        ...
        public static string PathCombine(string pA, string pB, string pC="", string pD = "", string pE = "")
        {
        ...
        public static bool CopyFile(string fromPath, string toPath, int bufferLength = 1024 * 1024 * 5)
        {
            ...
            if (bufferLength < fromFile.Length)
            {
                byte[] buffer = new byte[bufferLength];
                long copied = 0;
                while (copied <= fromFile.Length - bufferLength)
                {
                    toCopyLength = fromFile.Read(buffer, 0, bufferLength);
                    fromFile.Flush();
                    toFile.Write(buffer, 0, bufferLength);
                    toFile.Flush();
                    //流的当前位置  
