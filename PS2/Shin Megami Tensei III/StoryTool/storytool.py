...
class TextParser(object):
    def ParseBMD(self, data, tbl_name = "jp.TBL"):
        bmd_data_list = []
        bmd_string_list = []
            ...
            for j in xrange(sub_nums):
                ...
                while pos < len(c_data):
                    ...
                    if v0 == 0xf1:
                        ...
                        pos += 1
                bmd_string_list.append((code_name, j, current_string))
        return bmd_string_list

    def ParseBF(self, data):
        ms = BinaryReader(StringIO(data))
        ms.seek(0)
        ms.seek(0x10)
        nums = ms.ReadInt32()
    ...
    def ExportText(self, input_folder, dest_folder):
        ...
        for fn in fl:
            if fn[-4:].lower() == ".bmd":
                ...
                data = fp.read()
                bmd_string_list = self.ParseBMD(data)
                dest_name = fn.replace("\\", "__")
                dest = codecs.open(
                    "%s//%s.txt" % (dest_folder, 
                                    dest_name), "wb", "utf-16")
                for (code, i, string) in bmd_string_list:
                    dest.write("#### %s,%d ####\r\n%s\r\n\r\n" %
                               (code, i, string))
                fp.close()
    ...
    def ExportELF(self, input_elf, output_folder):
        special_list = [0x002e9c58] # special bmd using font1.tbl
        ...
        for (offset, size) in data_list:
            dest = codecs.open(
                    "%s//%s_%08x.txt" % (output_folder, 
                                    input_elf, offset), "wb", "utf-16")
            fp.seek(offset)
            data = fp.read(size)
        ...
        pass
tr = TextParser()
#tr.ExportText("img", "text")
tr.ExportELF("SLPM_654.62", "text")
