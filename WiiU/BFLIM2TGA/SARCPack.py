...
class SimpleArchive(object):
    def pack(self, folder, padding):
        ...
        v0 = folder + ".ARC"
        v0 = v0.split("\\")[-1]
        v0 = "import\\" + v0.replace("__" , "\\")
        v0_folder = "\\".join(v0.split("\\")[:-1])
        if not os.path.exists(v0_folder):os.makedirs(v0_folder)
        
        sarc = open(v0 , "wb") #SARC of the same name
...
def repack_files(fname):
    if not os.path.isdir(fname): #Not a folder, not gonna bother
        return
    test = SimpleArchive()
    test.pack(fname, 8192)
def main():
    fl = os.listdir("REPACK")
    for fn in fl:
        print(fn)
        repack_files("REPACK//%s"%fn)
