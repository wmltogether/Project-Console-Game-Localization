...
namespace psp_gxm.Compression
{
    /*
     * From
     https://github.com/xdanieldzd/Scarlet/blob/master/Scarlet/Drawing/Compression/DXTx.cs
     */
    ...
    internal static class DXTC
    {
        ...
        /// <param name="bUseVitaSwizzled"></param>
        /// <returns></returns>
        public static byte[] Decompress(EndianBinaryReader reader, int width, int height, PixelDataFormat inputFormat, long readLength, bool bUseVitaSwizzled = false)
        {
            ...
            for (int y = 0; y < height; y += 4)
            {
                for (int x = 0; x < width; x += 4)
                {
                    ...
                    if (bUseVitaSwizzled)
                    {
                        TextureTool.GetVitaSwizzledTransform(x / 4, y / 4, width / 4, height / 4, out rx, out ry);
    ...
    internal class DXT1Block
    {
        ...
        public DXT1Block(EndianBinaryReader reader, DXTxBlockLayout blockLayout)
        {
            ...
            Bits = DXTC.ExtractBits((((uint)bits_0 << 24) | ((uint)bits_1 << 16) | ((uint)bits_2 << 8) | (uint)bits_3), 2);
